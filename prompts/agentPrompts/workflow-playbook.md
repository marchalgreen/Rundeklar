...
Before merge, every epic must pass: 1. pnpm run validate → typecheck + lint (tsc + eslint) 2. pnpm build → includes Prisma generate and Next build 3. Manual smoke test
• Start dev server (pnpm dev)
• Open / and the target window or route
• Verify primary interaction and UI integrity

Optional but encouraged 4. UI validation — verify tokens, spacing, and glass consistency 5. Framer Motion audit — ensure animations respect duration/easing standards
...
🧭 Plan-First Output (format) 1. High-Level Approach – 3–6 bullets summarizing architecture and strategy 2. File-by-File Plan – paths and specific actions (create, modify, remove, refactor) 3. Diffs or Full Content – complete, paste-ready code for new files 4. Validation Steps – exact commands, URLs, and behavior to confirm success

Rules
• Respect guards.md (no schema edits without plan)
• Use Tailwind tokens + shadcn/ui components
...
Respect guards.md, use Tailwind tokens + shadcn/ui, and no schema edits unless migration steps are explicitly listed.
...
• No hard borders; rings + tokens only in docs (see design-tokens)
