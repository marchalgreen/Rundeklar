  ...
  return (
    <article className="space-y-8">
      <DocPageHeader
        eyebrow="Calendar"
        title="Quickstart"
        description="Create your first appointment, move/resize, and confirm cross-staff changes. All flows mirror the current store and API stubs."
      />

      <ol className="list-decimal pl-6 space-y-3 text-sm">
        <li>Open the Calendar window and switch to Day view.</li>
        <li>Click-drag on a staff lane to create a 30 min event (snaps to 15 min).</li>
        <li>Drag to move the event; drag edges to resize (min 15 min).</li>
        <li>Move to another staff in Day view — a confirmation will appear; accept to commit.</li>
        <li>
          Press <kbd>Enter</kbd> to open the editor for the selected event; <kbd>Esc</kbd> clears
          selection/focus.
        </li>
      </ol>

      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[hsl(var(--foreground))]">API smoke via curl</h2>
        <pre className="overflow-auto rounded-md bg-black/80 p-3 text-[12px] text-white ring-1 ring-[hsl(var(--line)/.12)]">
          {`# Create event (server returns id)
curl -sX POST http://localhost:3000/api/calendar/events \
  -H "Content-Type: application/json" \
  -d '{"title":"Demo","customerId":"c_1","staffId":"s1","start":"2025-01-01T08:00:00.000Z","end":"2025-01-01T08:30:00.000Z","status":"booked","serviceType":"eyeexam","createdBy":"system","updatedBy":"system","createdAt":"2025-01-01T08:00:00.000Z","updatedAt":"2025-01-01T08:00:00.000Z"}' | jq
`}
        </pre>
      </section>
      ...
      <section className="space-y-4 text-base leading-relaxed">
        ...
        <p className="text-[hsl(var(--muted))]">
          ...
          sandbox key with the <code>calendar:write</code> and <code>calendar:read</code> scopes.
          Copy the secret value — you will not be able to view it again.
        </p>
